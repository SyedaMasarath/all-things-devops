...
# Sensitive outputs are marked — they will not appear in plan logs.
################################################################################
...
output "kubeconfig_command" {
  description = "Run this command to update your local kubeconfig for the prod cluster"
  value       = "aws eks update-kubeconfig --region ${var.aws_region} --name ${local.cluster_name} --alias prod"
}
...
output "db_credentials_secret_arn" {
  description = "Secrets Manager ARN containing DB credentials (username, password, host, port, database)"
...
output "ecr_repository_urls" {
  description = "Map of ECR repository names to their full URLs (use as image.repository in Helm)"
